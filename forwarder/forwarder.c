...
*/
    ...
    if (retval < 0) {
        ...
        fprintf(stderr, "no config response\n");
        return -1;
    }
    ...
    //printf("file_length got nbytes %d: %d\n", nbytes, file_length);
...
//int tx_pkts_in_idx, tx_pkts_out_idx;
...
void gw_tx_service()
{
    ...
    if (beacon_guard)
        return; /* give priority to beacon transmission */
...
server_downlink_service()
{
    ...
    if (bytes_available > sizeof(user_buf)) {
        printf("downlink user_buf[] too small %d\n", bytes_available);
        return;
    }
    nbytes = read(sock, user_buf, bytes_available);
    if (nbytes != bytes_available) {
        printf("sock-read length mismatch: %d %d\n", nbytes, bytes_available);
        return;
    }
...
{
    struct timespec now, sleep_until;
    ...
    for (;;) {
        ...
        //usleep(10000);
        ret = clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &sleep_until, &rem);
        if (ret != 0) {
        ...
        if (connected_to_server && !have_config) {
            if (--retry_cnt <= 0) {
                if (request_config()) {
                    /* failed to get config */
                    retry_cnt = 400;
                } else {
                    printf("got config from server\n");
                    if (check_sx1301_config() < 0) {
                        /* check failed */
                        retry_cnt = 400;
                        printf("bad config from server\n");
                    } else {
                        /* check ok */
                        have_config = true;
                        printf("starting concentrator...\n");
                        i = lgw_start();
                        if (i == LGW_HAL_SUCCESS) {
                            MSG("INFO: [main] concentrator started, packet can now be received\n");
                            lgw_started = true;
                            if (first_pps()) {
                                printf("first pps failed\n");
                            } else
