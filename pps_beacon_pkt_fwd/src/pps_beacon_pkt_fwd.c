...
volatile bool _beacon_sent = false; /* wbr */
...
int pps_find_source(char* path, pps_handle_t* handle, int mode)
{
   pps_params_t params;
   int ret, capable_modes;
   ...
   if (ret < 0) {
		printf("time_pps_create() failed\n");
		return -1;
   }

   printf("found PPS source \"%s\"\n", path);

   ret = time_pps_getcap(*handle, &capable_modes);
   if (ret < 0) {
		printf("time_pps_getcap() failed\n");
		return -1;
   }

   if (!(capable_modes & PPS_CANWAIT)) {
		printf("no PPS_CANWAIT\n");
		return -1;
   }

   if ((capable_modes & mode) == mode) {
		printf("pps can set mode\n");
		ret = time_pps_getparams(*handle, &params);
		if (ret < 0) {
			printf("time_pps_getparams() failed\n");
			return -1;
		}
		params.mode |= mode;
		ret = time_pps_setparams(*handle, &params);
		if (ret < 0) {
			printf("time_pps_setparams() failed\n");
			return -1;
		}
   } else {
		printf("pps cant set mode\n");
		return -1;
   }
...
static uint16_t crc_ccit(const uint8_t * data, unsigned size) {
	...
	const uint16_t init_val = 0x0000; /* CCITT */
	uint16_t x = init_val;
	unsigned i, j;
...
int main(void)
{
	...
	if (pps_find_source("/dev/pps0", &pps_handle, PPS_CAPTUREASSERT) < 0) {
		MSG("ERROR: pps_find_source() failed\n");
		exit(EXIT_FAILURE);
	}
	...
	if (gps_enabled == true) {
		...
		i = pthread_create( &thrid_pps, NULL, (void * (*)(void *))thread_pps, NULL);
		if (i != 0) {
			MSG("ERROR: [main] impossible to create pps thread\n");
			exit(EXIT_FAILURE);
		}
	}
	...
	pthread_cancel(thrid_pps); /* don't wait for PPS thread */
	...
	while (!exit_sig && !quit_sig) {
		...
		while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
			...
			if (val != NULL) {
				...
				txpkt.rf_power = 10;	// TODO json config power limit
			}
...
void thread_pps(void)
{
	...
	while (!exit_sig && !quit_sig) {
		...
		//printf("pps seq: %lu, ", infobuf.assert_sequence);
		//printf("pps seq: %lu,  sec:%ld, nsec:% 6ld\n", infobuf.assert_sequence, infobuf.assert_timestamp.tv_sec, infobuf.assert_timestamp.tv_nsec);
		if (beacon_period > 0) {
			sec_of_cycle = (infobuf.assert_sequence + 1) % (time_t)(beacon_period);
			//printf("%d,%d xtal_correct_ok:%d ", sec_of_cycle, beacon_offset, xtal_correct_ok);
...
void thread_valid(void) {
	...
	while (!exit_sig && !quit_sig) {
		...
		if (ref_valid_local == false) {
			...
			pthread_mutex_lock(&mx_xcorr);
			if (xtal_correct_ok) {
				printf("xtal_correct_ok = false\n");
				fprintf(log_file, "xtal_correct_ok = false\n");
			}
			...
			if (init_cpt < XERR_INIT_AVG) {
				...
				if (!xtal_correct_ok) {
					printf("xtal_correct_ok = true\n");
					fprintf(log_file, "xtal_correct_ok = true\n");
				}
				xtal_correct_ok = true;
				_pps_first = true;
				pthread_mutex_unlock(&mx_xcorr);
