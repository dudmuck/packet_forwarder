    ...
    while (!exit_sig && !quit_sig) {
        ...
        for (i=0; i < nb_pkt; ++i) {
            ...
            if (ref_ok == true) {
                ...
                if (j == LGW_GPS_SUCCESS) {	/* thread_up() */
        ...
        while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
            ...
            if (i == 1) {
                ...
                if (val != NULL) {
                    ...
                    val = json_object_get_value(txpk_obj, "tmms");	/* thread_down() */
                    if (val == NULL) {
...
static void gps_process_sync(void) {
	...
	printf("gps:%lu.%09lu ", gps_time.tv_sec, gps_time.tv_nsec);
	printf("diff:%f\n", difftimespec(utc, gps_time));*/
